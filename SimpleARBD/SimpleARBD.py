...
class SetupARBD(object):
    ...
    def Get_hydrodynamic_properties(self, hydro_file_name_length_limit: int=20):
        '''
        Prepare files to compute the damping coefficients for our diffusible biomolecules using Einstein relation.
        p.s. There is a limit on the number of characters allowed in the name of files generated by HydroPro.
             We will automate this correction here.
        '''
        if self.Skip_parametrizing_diffusible in ["No", "no"]:
